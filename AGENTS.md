...
- Phase 1 (Playable Prototype). Done: M0, M1, M1b, M2 (`docs/M2_STATUS.md`), M2b - save migration and baseline compatibility (`docs/M2B_STATUS.md`), M2c - progression spine (`docs/M2C_STATUS.md`; the ratified model is `docs/PROGRESSION_AXIS_RECONCILIATION.md`), M3 - player, camera, movement, interaction, world cells (`docs/M3_STATUS.md`; its walk at 60 FPS was measured on RAZER in the Phase-1 closeout), the pre-M3b content-validation hardening, M3b - items, inventory, equipment (`docs/M3B_STATUS.md`), M3c - combat core (`docs/M3C_STATUS.md`, the generated `docs/M3C_TTK_TABLE.md`), M3d - creatures, perception, roles, corpses, respawn (`docs/M3D_STATUS.md`, the generated `docs/M3D_BEHAVIOUR_MATRIX.md`), M3e - basic magic: three formulas, Focus and Strain (`docs/M3E_STATUS.md`), M3f - gathering and blacksmithing: two nodes, two recipes, quality on the instance (`docs/M3F_STATUS.md`), M4 - the waystation's people: three NPCs, conversations as data, relationships, trade (`docs/M4_STATUS.md`), M5 - quests as objective graphs and the quest debugger, with the content bible's Quest 1 (`docs/M5_STATUS.md`), M6 - the content bible's four cells, Quest 2 and the companion, and the playable-prototype report (`docs/M6_STATUS.md`, the evidence in `docs/acceptance/m6/`). After M6 came the Phase-1 technical-audit remediation (`docs/PHASE1_AUDIT_REMEDIATION_STATUS.md`) and the Phase-A complete prototype; Phase 1 is technically closed and merged into `main` (2026-09-25): `docs/PHASE1_TECHNICAL_CLOSEOUT.md` is the integrated state (save schema 14), with the RAZER baseline measured and accepted. M3's 2 x 2 km capture (exit b) is still to run on RAZER.
- Phase 2: **M7 - Factions, Reputation, and Building v1 - in progress** (owner authorization 2026-09-25; status `docs/M7_STATUS.md`). The owner's rulings of 2026-09-24 are recorded as `docs/DECISIONS.md` D-13 (navigation is a derived, deterministic, headless domain grid; Godot navigation is never authoritative) and D-14 (building v1 is one storey). M7 migrates saves from schema 14 to 15.
...
- M7 (owner authorization, 2026-09-25): Claude is the implementing agent. It works only in the `G:\UNNAMED_M7` worktree on branch `claude/m7-factions-building`, cut from `main` at `a69693108b89d471cdb409a229ba0bdf78b08fa4`, pushes that branch, and keeps one persistent draft PR into `main` through E10. It never merges to `main` and creates no tag; the owner merges, and tags `m7` at the merge. Phase-B (visual overhaul) work is not an input to M7.
...
- `tests/Application.Tests/GameSaves/` holds committed game saves the game itself wrote (the M6 acceptance save, and M7's start saves). They are byte-exact: never edit them.
