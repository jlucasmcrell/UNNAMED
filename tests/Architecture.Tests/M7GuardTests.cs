...
public class M7GuardTests
{
    ...
    {
        string[] world = { "/Navigation.cs", "/Factions.cs", "/Building.cs", "/BuildingRules.cs", "/Errands.cs" };
        ...
        Assert.Contains(files, f => f.Path == "src/World/Runtime/Errands.cs");
        ...
        foreach (string binding in new[]
                 {
                     ...
                     "Bind(\"build_repair\", Key.T)", "Bind(\"work_order\", Key.Y)",
                 })
