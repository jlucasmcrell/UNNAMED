...
3. **`expected.json` is the current state the fixture must load to.** Change it only when the
   ...
   below.
4. The fixtures load against `content/` (fixture content 0.2.6) and `worldgen_profile.json`. Those are
   ...
   require (M3f). The writer packs are historical and are never edited, so they need not pass today's
