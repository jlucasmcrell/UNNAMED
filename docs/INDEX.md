...
**Phase:** 1 — Playable Prototype. M0, M1, M1b, M2, M2b and M2c are complete; M3 is implemented and waits on its RAZER performance measurement (`M3_STATUS.md`); M3b and M3c are complete (`M3B_STATUS.md`, `M3C_STATUS.md`, `ROADMAP.md`). The documents below are the Phase 0 architecture set; implementation is underway in `src/` and `tests/`.
**Root:** `G:\UNNAMED`
