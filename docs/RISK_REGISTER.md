...
**Earliest inexpensive validation.** Phase 1 or early Phase 2: place ~200 snapped pieces, place one companion and one NPC inside the structure, save, change a `content_version`-independent input, reload, and assert (a) piece count and transforms match, (b) the navmesh rebuild lets both actors path from the doorway to an interior socket, (c) the delta save grows sub-linearly with piece count. All three are automatable headlessly and cost one afternoon.
...
**Earliest inexpensive validation.** Phase 1/early Phase 2, and it needs the engine: place a snapping structure across a cell boundary, then ask a companion to path from one side to the other. Assert a complete path exists and is traversable, then force a cell unload/reload and repeat. The cheapest useful version is a single straddling wall with a doorway — the smallest structure that spans a seam and requires routing through it.
