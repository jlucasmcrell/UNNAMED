...
**Phase-1 audit remediation (2026-09-24, C-01).** `acquire_item` also takes `or_item_refs`: other items that count as the one wanted, typically what it is made into, so ore already smelted into a billet still counts as ore obtained (each listed once, never the `item_ref` itself). QST001 refuses an `acquire_item` whose counted items can be used up for good. An item is renewable when a daily node yields it, a returning creature's loot drops it, or a recipe makes it only from renewable items. For a counted item that is not renewable, every recipe consuming it must make something the objective also counts, and using the item must not spend it.
