...
4. **The architecture test in Milestone 1/2 asserts both**, before there is anything to violate: (a) `IWorldStateWriter` is not visible outside `Domain`, and (b) every component key is claimed by exactly one system. **Status:** `tests/Architecture.Tests` asserts (a), including that `ISystem.Configure` is internal. It also asserts that the world delta has no public mutation, that Domain/EntityRegistry/World/Persistence hold no static mutable state, and that only test assemblies see internals. (b) waits for the typed component-key registry, which does not exist yet; its first owner-system milestone must add both.
...
- **A delta is applied only to the baseline it was made against.** Every changed cell records its `baseline_hash`; the load regenerates, compares, and applies, rebases through a registered transition, or refuses - never a silent regeneration (`PERSISTENCE.md` §6.4). Content identity resolves definition IDs; it does not seed generation (M2b).
