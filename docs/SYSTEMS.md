...
- **Transient.** Formation slot, order-execution stack, combat micro-state. **Persistent (M7 reconciliation (2026-09-24)):** the persisted route a companion is walking, saved with its body; the search that produced it is transient.
...
- **M6 reconciliation (the owner's companion ruling).** The runtime `CompanionSystem` owns the roster (`StateSlice.Companions`, saved with the player, schema 12): each companion's order (follow or wait), condition (up or downed), health, and the trail they walk; their body is their NPC body, moved through `NpcSystem` (`PlaceNpc`). Recruiting is a conversation's `recruit_companion`; orders come from `OrderCompanionCommand` (a key) or a conversation's `order_companion`, and are refused with a reason for someone downed or not with the character. Following, a companion walks the character's trail - a mark a metre, straight for the farthest mark in clear view - at a walk, run or sprint by distance, and stands within 2.5 m; more than 30 m behind, or making no headway for 4 s, they are put down near the character on the trail or on a ring behind them (`CompanionCaughtUp`). Waiting, they stand. They fight creatures engaged with the character within reach of them and the character (while waiting, only what comes within 4 m), with their weapon's numbers and none of the character's; an engaged creature fights them instead of the character when they stand nearer by more than a metre (never with a charge), and their blows land through the same rules (`CompanionStruck`). Their kills earn the character no XP and count for no quest. At 0 health they are downed (`CompanionDowned`) and cannot talk; `ReviveCommand` within a hand's reach helps them up with 40% health (`CompanionRevived`); left 60 s they fall, and are back at the Ashen Waystone, whole and waiting (`CompanionFell`). They mend out of a fight. The character passes through a companion; creatures do not. Nothing caps the roster's size. Not built (owner ruling): contracts, wages, morale, companion progression and equipment, anchors, dismissal, personal quests, an affinity ladder.
...
**As built (M7).** S-27 owns the character's act log, what each faction knows of each act, and the standing each holds (points; the tier is derived from them when read and never stored), plus the factions' static relations, which only views read. Nothing is transient: no witness working set, no service cache, no reports in flight. The legal and offense state is not built; crime, bounty and pardon are Phase 3. `AddReputation` does not exist: standing moves only through acts a faction learns of.
- **K1, K5.** Knowledge rows are written only when the character tells a member: a dialogue reply carrying `report_act` sends `ReportAct` to `FactionSystem`, and the speaker's faction learns every logged act of that kind and subject, in ascending sequence, `reported`, via the speaker, `identified`.
- **K6-K10.** A faction knows at the tick a member is told, with no delay or per-member memory. A faction with no reaction row for an act stores nothing. A second report of an act already known changes nothing; an unidentified row upgrades once. Nothing else writes knowledge or standing, and no faction learns from another. An act no faction reacts to is never recorded (relevance is fixed at boot).
- **Acts.** `RecordAct` is dispatched only by the character's kill of a creature (`CreatureSystem.Die`, not a companion's kill) and by a switch the character sets (`InteractionSystem.Work`); the act log holds 256 and evicts the oldest first, leaving standing untouched.
- **Events.** `ActRecorded`, `FactionLearned` and `ReputationChanged` (with the tiers before and after, and `Via`). Views and tests only; no system subscribes. A new game and a load publish none.
- **M9.** The witnessed channel (a member who sees an act, identified within range or not) is specified in the M7 design (§5.3.4) and not built; its stored keys (`source`, `via`, `identity`, `knower`) already exist, so it needs no migration.
...
- **Transient.** Ghost/preview placement, snap candidates, navigation dirty rectangles (M7 reconciliation (2026-09-24); the domain grid of `D-13`), construction progress.
...
| Persisting live AI, pathing, or animation state | Rebuilt at load; persisting it creates load-order bugs and save bloat. M7 reconciliation (2026-09-24): search state, grids and caches are never persisted; a mover's committed route is mover state, saved with the body it moves. |
