...
- **Transient.** The whole store; at load it is rebuilt by regenerating the baseline from `(world_seed, worldgen_version, content_hash)` and applying the delta.
...
- **Events/interfaces.** `MarkDirty(entityId | cellId, reason)`, `GetDelta(cellId)`, `ApplyDelta`, `GenerateBaseline(worldSeed, worldgenVersion, contentHash)`; emits `CellDirtied`, `BaselineRegenerated`, `DeltaApplied`. **Hard invariant:** baseline generation from `(world_seed, worldgen_version, content_hash)` must be deterministic and version-stable; a content-version mismatch triggers migration, never silent regeneration (D-05, `RK-01`). The signature deliberately takes **all three** of seed, worldgen version and content hash — implementing it as `(seed, content_version)` alone would drop the two fields `RISK_REGISTER.md` `RK-01` identifies as the likeliest early save-corrupting bugs.
...
- **Owns.** Save manifest (save version, content version, seed, playtime, screenshot ref, checksum), the serialization walk over persistent system state, the migration chain, atomic write (temp file + fsync + rename) and rolling backup slots, autosave policy.
...
- **Events/interfaces.** `Save(slot, kind)`, `Load(slot)`, `Migrate(from, to)`, `VerifyIntegrity`; emits `SaveStarted`, `SaveCompleted`, `SaveFailed`, `LoadStarted`, `LoadCompleted`, `MigrationApplied`, `SaveCorrupted`.

  **The load sequence is NOT specified here.** It is owned by `PERSISTENCE.md` §7.4 and implemented as a **gated state machine**, so an out-of-order call is a state error rather than a silent misbehaviour. This file previously published its own inline sequence (`validate manifest → validate content version → apply migrations → build registry baseline → apply world delta → deserialize player/companion state → emit WorldLoaded`) which **omitted alias/tombstone resolution and derived-cache recomputation** and therefore contradicted the authority it deferred to. That inline sequence is **superseded**; implement §7.4, which is additionally ordered so that aliases resolve *before* migration and validation, and derived caches recompute *last*.
