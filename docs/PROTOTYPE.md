...
| Factions | 0 defined, 1 structural slot | `faction_id` field exists on NPCs and is saved; no faction has reputation logic. (M7 reconciliation (2026-09-24): as built, no `faction_id` field exists on NPCs or in any save. M7 adds an NPC `faction_ref` as static content, not saved; M7's E3 writes the as-built text.) | Cost of the field is zero; adding it later to a shipped save schema is not. As built (M7 E3): membership is the NPC definition's `faction_ref`, definition data that is not saved until something can change it. |
...
| C10 | **Revised by owner ruling (2026-09-24):** Phase-1 combat must demonstrate that melee, ranged combat, and each proof formula are functional and meaningfully useful in an appropriate combat situation. No single encounter is required to force every combat tool. The March Spear is not weakened to make the den need everything. *(Superseded wording, kept as history: kill the den pack using only sword, bow, and the three spells, and the three spells must each be* used *to succeed under the intended tuning.)* | Melee, ranged combat or a proof formula does not work, or is useless in every fight it suits. *(Superseded: any spell is never needed, or one spell alone trivialises the pack.)* |
...
| A-2 | Faction reputation is out of prototype scope (`PHASE_0.md` STEP 15 does not list it), but `faction_id` appears in the NPC and save schemas from day one so no save migration is needed later. (M7 reconciliation (2026-09-24): this was never built; M7 adds `faction_ref` as static NPC content and the player's faction ledger in its own schema step.) Relationship changes are stored and clamped with exactly one observable effect (a dialogue line); the relationship *system* is vertical-slice work. | Schema field cost is near zero. As built (M7 E3): membership is the NPC definition's `faction_ref`, definition data that is not saved until something can change it. |
