...
**What combat state is kept, and what is not (the contract; the owner's rulings on the M7 E8.5 STOPs).** Two kinds of state are distinguished.
- **Presentation and transient state is not persisted.** Animation, effects, sounds being played, presentation timers, events already published, and runtime objects are never restored by a load: presentation derives what it shows from the loaded state.
- **Authoritative state whose omission would change the deterministic continuation is persisted, when the owning system requires it.** As of M7 that is the character's vitals (schema 16, above); a creature's charge cooldown, stagger and stagger immunity, and the sounds waiting to be heard (schema 14); and a creature's ordinary attack in progress (schema 17, §5.3). Each was added with a ruling and a failing save-and-continue equality that showed it was needed.
- **Still not kept, by the policy as of M7.** The character's own action in progress: a swing, dodge, working or stagger under way, a raised guard, the stagger immunity after a stagger, and the last blows kept for a death recap. A creature's charge run. A companion's action in progress and its next-attack tick. A load resumes each at rest. A save taken while one of them is under way can therefore go on differently from the unsaved world, and the equality of a save and its continuation holds only for saves taken outside them. M7 does not widen the kept set beyond the creature's ordinary attack: a further case needs its own ruling, and a failing equality to show it.
...
**Creature records (schema 8).** A spawner's creature that differs from its baseline - moved, wounded, dead, gone, or holding a mind other than rest - is stored in the section's `creatures` list, keyed by its derived `spawner#member` key and carrying its generation (the `generation_seq` above). It records the `instance_id`, `def_id`, `host_cell` and the host cell's `baseline_hash`, the condition (`alive`, `corpse`, `gone`), position, facing, health, the tick it died and the absolute tick it is due back, and its mind: awareness, whether and where it knows its target to be, when it last saw it, its search deadline, and whether it has called. A record at baseline is not stored (§5.6); load merges on the key, so a dead creature stays dead. A corpse's contents are an ordinary changed container once the player has touched them. Definition IDs go through the definition-ID pass. Idle wander and patrol derive from the world tick, so they need no storage. An ordinary attack in progress is kept from schema 17 (below).

**Creature continuation and pending sounds (schema 14; the Phase-1 technical audit, L-06).** Each creature record also carries what its next ticks depend on (`continuation`): `next_charge_tick`, the first tick it may charge again; `stagger_immune_until`, the first tick a blow may stagger it again; and the stagger it is in - a charger's stun among them - as `staggered_tick` and `stagger_lasts_ticks` (0 for the usual length). Each is written only while it still matters, so a creature over its cooldown, immunity or stun has the record of one that never had them. The section's `noises` list holds the sounds made on the last tick that creatures hear on the next - a blow, and a howl with its caller's kind (`caller_kind`, through the definition-ID pass) - in the order they were made. A save therefore goes on as the unsaved world would: a stunned boar stays down, a charger waits out its cooldown, and a howl made on the tick of the save still brings the pack. A charge's run is still not kept.

**A creature's attack in progress (schema 17; the owner's ruling on the second M7 E8.5 STOP).** The continuation also carries the creature's ordinary attack in progress (`attack`), or nil. It holds `start_tick`, the absolute tick the attack began, and `struck`, the instance ID of the body it has already landed on (the character or a companion), or nil while it has not landed. The running world then goes on after a load on the same ticks: the windup's remaining ticks, the impact, the recovery, and the bleed a bite starts. The Crossing Workshop's step 10 saves with a wolf one tick from biting, and needs exactly this. The attack is written only while it still matters, from its windup to its last tick of recovery. What it is built from, traced through the attack path:
- **The attack itself.** It is the creature definition's one ordinary attack: the first of its `attack_set`; a second ability may only be a charge. It is taken from the definition on load, so it is not stored.
- **The target.** None is stored. The creature picks its foe at every tick, the character or a nearer companion who is up, and strikes whichever it faces at the impact tick. A restored attack does the same from the loaded state, so no stored target can go stale.
- **The randomness.** None is chosen at the attack's start. The blow's roll is keyed at impact on the creature, the body and the tick (`RngChannel`), so the saved state is enough.
- **`struck`.** It only marks a swing as spent, since a swing lands once (L-23). No identity is looked up at load: a landing that names someone no longer in the world still marks the swing spent, as it was.
- **Refused as corrupt.** An attack on a creature that is not alive, or alongside a stagger (a creature does one thing at a time), or begun before tick 0, or landed on anything but a character or an NPC. Like any impossible creature state, the record is rejected alone and reported, never repaired. A `struck` that is not an instance ID fails the section's decode.
- **After a content change.** An attack whose phases are over by the load's first tick, because the ability was shortened, ends on that tick.
- **Not kept.** Animation, sounds and the `AttackStarted` event already published: presentation reads the creature's phase from its view.
...
**Errands at world start (E9).** An errand's pose is its NPC's body: the NPC stands there after a load, and the errand mover writes both in the same tick (G18). No tier gate and no conversation reach the mover, so a save taken mid-walk goes on the same, near the character or far from them. Before the NPCs are placed, each loaded errand is checked, and each repair is reported as an errand line after the piece lines of the structure audit:
- an errand whose NPC is a saved companion is dropped, since an NPC is never both (G23);
- an errand whose NPC has no place in the region is dropped;
- one whose NPC's place has moved to another cell is re-hosted there;
- one whose work place is no longer an intact station walks home;
- one at work but off its work anchor walks back to it.
...
| 16 -> 17 | Every creature record's continuation gains its ordinary attack in progress (the owner's ruling on the second M7 E8.5 STOP). An older save kept none, so its creatures resume with no attack in progress, exactly as every older save has always loaded. This is the compatibility limitation: a schema-16 save taken mid-attack loses the attack, and its creature begins again and lands late. What was not kept cannot be reconstructed |
...
**Schema 17's tests.**
- `CreatureAttackContinuationTests` (Application). A wolf bites a character who stands still. The world is saved as the windup begins, one tick before the bite lands, on the tick it lands, in its recovery, and with no attack in progress. Each save is loaded and stepped beside the unsaved world for the rest of 600 ticks: creatures, pools, effects and creature records are compared each tick, every bite and bleed must land on the never-saved world's ticks, and every field is compared at the end. The four attack moments failed before the fix. Also: a schema-16 save taken mid-attack loads with none and its wolf begins again, the documented limitation; and a landing that names someone absent stays spent.
- `BuildingAcceptanceTests.CrossingWorkshop_10`, unchanged, is the case that required it.
- `Schema17Tests` (Persistence). The attack round-trips exactly: in its windup, landed on the character, landed on a companion. Each impossible attack is rejected with its record while the rest loads. A `struck` that is no instance ID is corrupt. The 16 -> 17 step alone gives no attack and changes nothing else, the same bytes every time.
- `WorldDeltaTests`. The attack moves its cell's digest, and the load rule refuses each impossible attack.
- T-16 over v1-v17.
