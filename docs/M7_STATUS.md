...
| **Investigated 2026-09-26 (the owner's ruling on the E8.5 STOP).** Cause: a background save runs as a continuation on the shared .NET thread pool (`GameSession`, `_writer.ContinueWith(..., TaskScheduler.Default)`). The test gives the autosave 5 s (500 × 10 ms sleeps) to reach its first hook. Instrumented (not committed), the hook is normally reached in 11-25 ms. Only the class's first test is slow, while the other test classes run in parallel: 19 ms, 4,675 ms, and 13,195 ms in the failing run, with 17 pool threads busy and 10 work items queued, and the 5 s sleep loop itself taking 8.4 s of wall clock. Not caused by schema 16: on the commit before it (`9e3ef79`), unmodified, it failed in 3 of 4 full Application runs; on `1dd66e7` it failed in 1 of 3 and passed in the clean-worktree full run. Not a game defect: in the game the pool is idle and the save starts at once. It is a test that assumes a free thread pool. The fixes, for the owner: give the save writer its own thread (`TaskCreationOptions.LongRunning` or a dedicated thread), or give the test a longer budget. Neither is made: the test is Phase 1's, and the earlier ruling keeps it unedited | Here |
