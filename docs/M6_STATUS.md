...
- The overnight re-check: `dotnet test` 675 passed, 0 failed; lint 0 errors; the headless smoke `PASS`; the acceptance run recorded again, relaunched (412 fields, 0 differences) and replayed (byte-identical); the perf capture on ASTRAL clean (above); the windowed `--ui-shots` run exits 0 with the husk on the quarry floor.
...
- M6 part 1 `8935b8d`, part 2 `4df79ea`, part 3 `f3a064e`, part 4 `708bb8d` (this report), the feel-test row `6742e24`, and the overnight re-check (the husk's patrol, the perf walk, the evidence recorded again) - on `claude/phase1`, draft PR #1; nothing merged to `main`.
- The worktree is clean after the re-check's commit.
