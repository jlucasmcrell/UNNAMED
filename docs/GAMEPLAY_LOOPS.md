...
graph TD
    ...
    PROGRESS -->|"competence, ability,<br/>mastery, build shape"| FIGHT
    PROGRESS -->|"profession rank,<br/>quality ceiling"| CRAFT
...
- **Outputs** — character level, attribute points, skill ranks, abilities/talents, weapon mastery, magic mastery, profession rank, reputation. Per `D-09` these are **orthogonal axes with distinct questions**: level grants *breadth*, skills grant *competence*, masteries grant *specialization*, professions grant *production*, reputation grants *access*.
- **Reward entry points** — XP awards; level-up events; ability unlocks; mastery thresholds that change a weapon's move set rather than only its damage; profession ranks that raise a quality ceiling.
- **Drains** — opportunity cost is the primary drain and must be real: the charter forbids "fake choices where every character eventually unlocks everything." Additional drains: respec cost or scarcity, training time and money, gear requirements, and the fact that specializing closes routes as well as opening them.
- **Handoffs** — to `FIGHT` (competence and capability), `CRAFT` (profession rank and quality ceiling), `RECRUIT` (slots and contract leverage), `EXPLORE` (survivability at range from safety), `GATHER` (yield and rare-node access).
...
- **Inputs** — materials (`GATHER`); a recipe or the knowledge to discover one (`QUEST`, `EXPLORE`, trainers); a station (`BUILD`); profession rank, tooling, and quality ceiling (`PROGRESS`); optional craftspeople (`RECRUIT`).
...
- **Reward entry points** — successful crafts; quality rolls driven by material properties and profession rank; experimental discovery of new recipes; the charter's requirement that "crafting should not become irrelevant once dungeon loot appears," satisfied by letting master craftsmen reach equipment that competes with and eventually exceeds ordinary loot; and artifact-level crafting at the top of the profession (charter §12).
...
8. **`PROGRESS` must not be convertible into `CRAFT` output, or into `RETURN` access, without the respective loop's inputs.** No buying profession rank with gold alone; no buying fast travel with level. `D-09` states this: a character cannot convert one axis into another.
...
| **E-7** | **Progression shortcut: buying power with gold, or buying `RETURN` access with level.** Skips the loop that is supposed to produce the capability. | Wealth from any loop is fungible, so the natural player behaviour is to convert it into whatever axis is cheapest — which collapses `D-09`'s orthogonal axes into one. | `D-09`'s rule is absolute: a character cannot convert one axis into another. Gold buys **goods and services** (equipment, crafting fees, hireling wages, building materials), never **rank** — not profession rank, not reputation, not mastery, not fast-travel permission. Reputation and access are earned through deeds and faction relationship, never purchase. | Content review checklist against `D-09`, plus a Phase-2 grep-level test: no content definition may grant a mastery, profession, or reputation rank purely in exchange for currency. Cheap, and it prevents the drift before it exists in data. |
...
- **Raw power** — level, attributes, equipment, mastery, magic mastery.
...
| **PROGRESS** | XP, levels, basic skills, one ability path; a first real build decision. | Several skill trees; multiple builds; weapon and magic mastery; reputation; factions. | Mastery progression past the normal cap; prestige systems; transformation abilities. |
